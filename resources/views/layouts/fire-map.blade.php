            ...
            <header class="z-20 flex h-12 shrink-0 items-center justify-between gap-4 border-b border-slate-200/90 bg-white/95 px-4 backdrop-blur-xl dark:border-slate-700/60 dark:bg-slate-950/90">
                <div class="flex items-center gap-3">
                    ...
                    <div class="hidden items-center gap-1.5 sm:flex">
                        <flux:icon.map-pin class="size-3.5 text-orange-600 dark:text-orange-400" />
                        <span class="text-sm font-semibold tracking-tight text-slate-800 dark:text-slate-100">{{ __('Focos de incêndio') }}</span>
                    </div>
                </div>

                <div class="flex items-center gap-2">
                    <flux:button href="{{ route('dashboard') }}" icon="arrow-left" size="xs" variant="ghost">
                        {{ __('Painel') }}
                    </flux:button>
