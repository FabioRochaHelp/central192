...
/** Evita rajadas de requisições ao Traccar após falhas consecutivas. */
final class TraccarCircuitBreaker
{
    private const string FAILURES_KEY = 'traccar.circuit.failures';

    private const string OPEN_UNTIL_KEY = 'traccar.circuit.open_until';
    ...
    {
        $threshold = max(1, (int) config('traccar.circuit_breaker_threshold', 3));
        $cooldownMinutes = max(1, (int) config('traccar.circuit_breaker_cooldown_minutes', 5));
